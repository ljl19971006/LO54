...
<ul class="nav nav-tabs">
    <li role="presentation" ><a href="http://localhost:8099/dateandlocation">bydateandlocation</a></li>
    <li role="presentation" class="active"><a href="http://localhost:8099/dateindex">bydate</a></li>
...
<div id="search">
    <div id="search-wrap">
        ...
        <input class="search-content" type="text" placeholder="search.." name="word" id="word">
...
<table data-toggle="table" id="ttable">
    ...
    <tr>
        ...
        <td>
            ...
            <button type="button" class="btn btn-primary btn-lg" data-toggle="modal" data-target="#exampleModal" onclick="displaysessionid(this,this.id)">
                inscrire
            </button>
            <!-- Modal -->
        </td>
...
<script >
    ...
    function search() {
        ...
        var contexts=document.getElementById("word").value;
        // alert(document.getElementById("word").value);
        var URLa='http://localhost:8099/byDate?byDate='+contexts
        var vm=this;
        $.ajax({
            type:'get',
            // async:false,
            url: 'http://localhost:8099/'+options+'?'+options+'='+contexts,
    ...
    function displaysessionid(obj,id) {

        var td_content = $(obj).parents("tr").children("td");  //获取当前行中的所有td值
        var content = td_content.eq(0).text();
        // var session =document.getElementById("sessionid").innerText;
    alert(content)
    }
